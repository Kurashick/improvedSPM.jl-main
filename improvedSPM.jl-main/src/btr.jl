...
function solveDifferentiableBTR(
    images::Vector{Surface}, tip_size::Integer, max_epoch::Integer, mu::Real, lambda::Real;
...
"""
"""
    function diffmaps(tip_data)
        #上下左右に1行・列動かしたものを用意する
        #端の列はtipを保持(MSEをとる時に影響のないように)
        up=vcat(tip_data[2:end, :],tip_data[end:end, :])
        down=vcat(tip_data[1:1, :],tip_data[1:end-1, :])
        left=hcat(tip_data[:, 2:end], tip_data[:, end:end]) 
        right=hcat(tip_data[:, 1:1],tip_data[:, 1:end-1])
        #上との差＝下に移動したtip-元tipであることに注意
        return down-tip_data, up-tip_data, right-tip_data, left-tip_data
    end        
        
    function loss(v_mat_cp, v_mat)
        zeromap=zeros(Float32, tip_size, tip_size)
        tip_data = tip.matrix
        opened = [MDT.idilation(MDT.ierosion(mat_cp, tip_data), tip_data) for mat_cp in v_mat_cp]
        i_loss=Statistics.mean(Flux.Losses.mse.(opened, v_mat))
        upward_diff, downward_diff, leftward_diff, rightward_diff =diffmaps(tip_data)         
        up_d_loss=Statistics.mean(Flux.Losses.mse.(upward_diff, zeromap))
        down_d_loss=Statistics.mean(Flux.Losses.mse.(downward_diff, zeromap))
        left_d_loss=Statistics.mean(Flux.Losses.mse.(leftward_diff, zeromap))
        right_d_loss=Statistics.mean(Flux.Losses.mse.(rightward_diff, zeromap))
        return i_loss + (up_d_loss + down_d_loss + left_d_loss + right_d_loss)
    end

    当初はこれでやっていたが，loss(左)＝loss(右)を利用してコード削減
"""

    function diffmaps(tip_data)
        #上左に1行・列動かしたものを用意する
        #下端・右端の列はtipを保持(MSEをとる時に影響のないように)
        up=vcat(tip_data[2:end, :],tip_data[end:end, :])
        left=hcat(tip_data[:, 2:end], tip_data[:, end:end]) 
        #下との差＝上に移動したtip-元tipであることに注意
        return up-tip_data,left-tip_data
    end        
    ...
    function loss(v_mat_cp, v_mat)
        ...
        opened = [MDT.idilation(MDT.ierosion(mat_cp, tip_data), tip_data) for mat_cp in v_mat_cp]
        i_loss=Statistics.mean(Flux.Losses.mse.(opened, v_mat))
        downward_diff, rightward_diff =diffmaps(tip_data)         
        down_d_loss=Statistics.mean(Flux.Losses.mse.(downward_diff, zeromap))
        right_d_loss=Statistics.mean(Flux.Losses.mse.(rightward_diff, zeromap))
        return i_loss + mu*(down_d_loss + right_d_loss)
    end
    # 訓練するモデルを与える
    ps = Flux.params(tip)
